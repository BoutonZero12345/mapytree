...
export default function SortableBlock({ block, index, travelInfo, isLast, nextBlockId, nextBlockMode, nextBlockRouteIndex }) {
    ...
    return (
        <div ref={setNodeRef} style={style}>
            ...
            <div className={`bg-white border ${isDragging ? 'border-blue-500 shadow-xl' : 'border-gray-200'} rounded-xl p-4 shadow-sm flex flex-col gap-2`}>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-1 shrink-0">
                        <button onClick={() => setIsEditingName(!isEditingName)} className="p-1 text-gray-400 hover:text-blue-600 rounded-md transition-colors"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></button>
                        <button onClick={() => { if (window.confirm("Supprimer ?")) deleteBlock(block.id); }} className="p-1 text-gray-400 hover:text-red-600 rounded-md transition-colors"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
                    </div>
                </div>

                {/* Détails (Durée, Budget, Notes) */}
            ...
            {/* Affichage du module de transport s'il y a un lieu suivant */}
            {!isLast && (
                <TransportSelector
