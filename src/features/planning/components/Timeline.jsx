...
export default function Timeline() {
    ...
    // NOUVEAU : Filtrage par catégorie & Toasts premium
    ...
    const filteredFavorites = useMemo(() => {
        if (!activeCategoryFilter) return favorites;
        return favorites.filter(fav => fav.categoryId === activeCategoryFilter);
    }, [favorites, activeCategoryFilter]);
    ...
    const handleAddCategory = (e) => {
        ...
        if (newCategoryName.trim()) {
            ...
            showToast(`Catégorie "${newCategoryName.trim()}" créée !`);
        }
    };

    const handleAddFavoriteToPlanning = (fav) => {
        // Validation doublons
        const alreadyExists = activeBlocks.some(b => b.placeId === fav.placeId);
        if (alreadyExists) {
            const confirmAdd = window.confirm(`"${fav.name}" est déjà présent dans ce scénario. Voulez-vous tout de même l'ajouter en double ?`);
            if (!confirmAdd) return;
        }
        addBlock(fav);
        showToast(`"${fav.name}" ajouté au planning`);
    };
    ...
    return (
        <div className="flex flex-col h-full bg-white">
            ...
            {/* Barre de contrôle : Heure de départ & Bouton d'affichage du Déroulé */}
            <div className="flex items-center justify-between gap-3 mb-4 bg-slate-50 border border-gray-150/70 p-3 rounded-2xl shrink-0">
                {/* Sélecteur d'heure "Départ à partir de" */}
                ...
                {/* Bouton pour afficher/masquer le déroulé */}
                <button
                ...
                <div className="flex-1 overflow-y-auto flex flex-col gap-4 pb-4 no-scrollbar">
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between px-1">
                            ...
                            <button 
                                ...
                                className="text-blue-600 hover:text-blue-700 p-1 rounded-md hover:bg-gray-50 transition-colors"
                                title="Ajouter une nouvelle catégorie"
                            >
                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                            </button>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 px-1">
                            <button
                                ...
                                className={`px-2.5 py-1 rounded-full border text-[10px] font-extrabold transition-all ${
                            ...
                            {categories.map(cat => {
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-extrabold transition-all group cursor-pointer ${
                                            isSelected 
                                                ? 'shadow-sm font-black scale-102' 
                                                : 'hover:bg-gray-50/50'
                                        }`}
                                        style={{ 
                                        ...
                                        <button 
                                            onClick={(e) => {
                                                ...
                                                if (window.confirm(`Supprimer la catégorie "${cat.name}" ?`)) {
                                                    ...
                                                    if (activeCategoryFilter === cat.id) setActiveCategoryFilter(null);
                                                    showToast(`Catégorie "${cat.name}" supprimée`);
                                                }
                                            }}
                                            className={`transition-opacity hover:text-red-500 ${isSelected ? 'text-white/80 hover:text-white' : 'opacity-0 group-hover:opacity-100'}`}
                                        >
                    ...
                    {filteredFavorites.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-gray-400 text-center p-8">
                            ...
                            <p className="text-sm font-medium">Aucun favori trouvé.</p>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-2">
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div 
                                                ...
                                                onClick={() => { if(!isEditing) { handleAddFavoriteToPlanning(fav); } }}
                                            >
                        ...
                        {isFooterCollapsed ? (
                            ...
                            >
                                <div className="flex items-center gap-4 text-xs font-bold text-gray-500">
                                    <span className="flex items-center gap-1.5">
                                        <svg className="text-gray-400 shrink-0" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                                        <strong className="text-gray-800 font-extrabold">{formatDuration(totalDurationMinutes)}</strong>
                                    </span>
                                    <span className="text-gray-300">|</span>
                                    <span className="flex items-center gap-1.5">
                                        <svg className="text-gray-450 shrink-0" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>
                                        <strong className="text-gray-850 font-extrabold">{totalBudget} €</strong>
                                    </span>
                                </div>
                            ...
                            >
                                ...
                                <div className="flex justify-between items-center text-xs">
                                    <span className="text-gray-505 font-medium">Durée totale cumulée :</span>
                                    <span className="font-extrabold text-gray-800">{formatDuration(totalDurationMinutes)}</span>
                                </div>
                                <div className="flex justify-between items-center text-xs">
                                    <span className="text-gray-550 font-medium">Budget total estimé :</span>
            ...
            {toastMessage && (
                <div className="fixed bottom-4 left-4 right-4 md:left-[20px] md:right-auto md:w-[350px] z-50 bg-slate-900/90 text-white px-4 py-3 rounded-xl shadow-xl flex items-center justify-between gap-3 animate-in slide-in-from-bottom-2 duration-300 backdrop-blur-md border border-white/10 text-xs font-bold">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setToastMessage(null); }} className="text-white/70 hover:text-white p-0.5 rounded">
                        ✕
                    </button>
                </div>
            )}
